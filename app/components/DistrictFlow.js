...
// 입력 폼을 "1. 지역 선택" -> "2. 필요 예산(자동 계산)" 2단계 슬라이드로 나눠 보여줍니다
// (①모드와 동일한 이유 — 휴대폰에서 스크롤이 너무 길어지는 문제 개선, README 29번).
const DISTRICT_WIZARD_LABELS = ["지역 선택", "예산/조건"];
const DISTRICT_TOTAL_STEPS = DISTRICT_WIZARD_LABELS.length;
...
export default function DistrictFlow() {
  ...
  function goToNextStep() {
    setStep((s) => Math.min(s + 1, DISTRICT_TOTAL_STEPS));
  }
  ...
  return (
    <div>
      ...
      <form className="panel" onSubmit={handleSearch} ref={formTopRef}>
        <WizardProgress step={step} labels={DISTRICT_WIZARD_LABELS} />
        ...
        <div className="wizard-step" key={step}>
          ...
          {step === 2 && (
            <>
              ...
              {!estimate ? (
                <p className="note">
                  {estimating ? "예산을 계산하고 있어요..." : "1단계에서 지역을 먼저 선택해 주세요."}
                </p>
              ) : (
        ...
        <div className="wizard-nav">
          ...
          {step < DISTRICT_TOTAL_STEPS ? (
            <button type="button" className="wizard-btn wizard-btn-next" onClick={goToNextStep}>
              다음 →
            </button>
          ) : (
            <button className="submit-btn" type="submit" disabled={searching || estimating}>
