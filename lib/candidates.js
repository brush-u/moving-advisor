...
// 예전엔 여기서 desiredRooms/desiredBathrooms(희망 방·화장실 개수)로도 걸러냈는데, 그 값은
// 실제 데이터가 아니라 lib/roomEstimate.js가 전용면적만 보고 "이 정도 나올 것"이라고 추정한
// 값입니다(국토부 실거래가 API 자체에 방/화장실 개수 필드가 아예 없습니다). 필터로 쓰면 추정이
// 틀렸을 때 실제로는 조건에 맞는 매물을 조용히 숨겨버릴 수 있어서(사용자가 정확하지 않다고
// 지적), 이 필터는 없앴습니다. 대신 추정 결과 자체(layout.label, 예: "2~3룸 · 욕실 1개(추정)")는
// toCandidate()에서 매물 카드에 참고 정보로만 계속 보여줍니다 — "추정"이라고 표시되어 있어
// 걸러내는 용도로 오인될 위험이 없습니다.
function applyConditionFilters(raw, { maxBuildAge, budgetMin, budgetMax }) {
...
/**
 * 추천된 자치구 안에서 사용자가 지정한 집 조건(주택유형/평형/준공연식)에 맞는 실제 거래 단지
 * 후보를 찾아 반환합니다. 개별 단지 데이터는 참고값으로 대체할 수 없으므로, MOLIT_SERVICE_KEY가
 * 없으면 항상 빈 배열을 반환합니다 (호출부에서 안내 문구를 보여줍니다).
 *
