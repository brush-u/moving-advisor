...
let insecureMolitAgent = null;
function getMolitDispatcher() {
  if (process.env.MOLIT_ALLOW_INSECURE_TLS !== "1") return undefined;
  if (!insecureMolitAgent) {
    insecureMolitAgent = new UndiciAgent({ connect: { rejectUnauthorized: false } });
  }
  return insecureMolitAgent;
}

async function throttledMolitFetch(url, attempt = 0) {
  await scheduleMolitSlot();
  const dispatcher = getMolitDispatcher();
