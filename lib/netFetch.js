 ...
 */
/**
 * Cloudflare Workers 런타임(workerd) 위에서 실행 중인지 확인합니다. Cloudflare 공식
 * 문서가 안내하는 방법으로, Workers 런타임은 `navigator.userAgent`를 항상
 * `"Cloudflare-Workers"`로 고정해 둡니다(일반 Node·브라우저에는 이런 값이 없습니다).
 *
 * 아래 MOLIT_ALLOW_INSECURE_TLS 우회 기능(자체 서명 인증서로 TLS 검사를 하는 회사망을
 * 우회하는 용도)은 실사용자의 로컬 PC/회사망 환경에서만 의미가 있습니다 — Cloudflare
 * Workers가 실제로 외부 API를 호출할 때는 Cloudflare 자체 네트워크를 통해 나가므로
 * 애초에 사용자의 회사망 TLS 검사를 거치지 않습니다. 게다가 workerd의 fetch 구현체는
 * (nodejs_compat를 켜도) undici의 `dispatcher`/`rejectUnauthorized` 같은 커스텀 TLS
 * 옵션 자체를 지원하지 않아서, 이 우회 기능을 그대로 두면 "ERR_OPTION_NOT_IMPLEMENTED
 * — The options.rejectUnauthorized option is not implemented" 오류로 요청이 아예
 * 실패합니다(README 31번 참고 — 실사용자가 `.env.local`에 설정해 둔
 * MOLIT_ALLOW_INSECURE_TLS=1이 Cloudflare 쪽 `.dev.vars`/시크릿에도 그대로 남아있어
 * 실제로 겪은 문제입니다). 그래서 Workers 런타임에서는 이 환경변수 값과 상관없이 항상
 * 우회를 건너뛰고 일반 fetch를 씁니다.
 */
